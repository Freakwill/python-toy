...
def transition(img1, img2, func, N):
    """To generate transition effect from img1 to img2
    
    Args:
        img1, img2 (ndarray): An array representing an image
        func (callable): A transition functions
        N (number): the number of frames
    
    Returns:
        list: Frames, a sequence of images(arrays)
    """
    u, v = img1.shape[:2]
    a = []
    for h in range(N):
...
def jpg2video(images, path, fps, repeat=1):
    ...
    Arguments:
        ...
        path {str} -- the video will be saved in the path
        fps {number} -- frame per second
        repeat {int} -- number of repeating
    """
...
def video_add_mp3(video_file, mp3_file):
    """Add audio to a video
    
    Arguments:
        video_file {str|Path} -- path of video
        mp3_file {str} -- path of audio file
    """
    if isinstance(video_file, str):
        video_file = pathlib.Path(video_file)
    outfile_name = video_file.with_suffix('.add-mp3.mp4')
    subprocess.call('ffmpeg -i ' + str(video_file)
                    + ' -i ' + str(mp3_file) + ' -strict -2 -f mp4 '
                    + str(outfile_name), shell=True)

if __name__ == '__main__':
    PATH = pathlib.Path('/Users/william/Pictures/travel/')
    ...
    outfile = 'out.avi'
    an.make(images, outfile=outfile, repeat=2)
    video_add_mp3(video_file=outfile, mp3_file='paomo.mp3')
